...
class RegionalOfficeController {
  ...
  static async createRegionalOffice(req, res) {
    try {
      ...
      if (existingOffice) {
        return res.status(400).json({ error: 'Regional office already exists in this organization' });
      }
      ...
      console.error('Error creating regional office:', error.response ? error.response.data : error.message);
      return res.status(500).json({ error: 'Failed to create regional office' });
    }
  }
  ...
  static async getRegionalOffices(req, res) {
    try {
      ...
      return res.status(500).json({ error: 'Failed to fetch regional offices' });
    }
  }
}
